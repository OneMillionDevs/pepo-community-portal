...
const write = ({branch = [], payload = {}, writeType = ''}) =>
  new Promise((resolve, reject) => {
    let objectReference;
    const path = PathReference(branch);
    switch (writeType) {
      case 'push': // https://firebase.google.com/docs/reference/js/firebase.database.Reference#push
        try {
          objectReference = path.push(payload);
          objectReference.update({uid: objectReference.key}); // Add the item unique identifier (uid) to new entry;
          resolve(objectReference);
        } catch (e) {
          reject(new Error(e.message));
        }
        break;
      case 'update': // https://firebase.google.com/docs/reference/js/firebase.database.Reference#update
        try {
        ...
        break;
      case 'set': // https://firebase.google.com/docs/reference/js/firebase.database.Reference#set
        try {
      ...
      default:
        reject(
          new Error(
            'error: databaseWrite failed because writeType default is null. Please select push, update or set.',
          ),
        );
    }
  });

const search = ({branch = [], boundaries = {}, order = {}}) =>
  new Promise((resolve, reject) => {
    const dataNew = [];
    ...
    if (
      boundaries &&
      boundaries.limitToFirst &&
      boundaries.limitToFirst < 500
    ) {
    ...
    path.on('child_added', (snapshot, childKey) => {
      return snapshot
        ...
        : reject(
            new Error(
              `Firebase Error: the admin.database.ref in databaseSearch() is originating the error.`,
            ),
          );
    });
...
const isObject = obj =>
  Object.prototype.toString(obj) === '[object Object]' ? true : false;
